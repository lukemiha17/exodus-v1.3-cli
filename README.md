...
- **#60 (P0 — single most important break)** — **Writing from a competitor ad fails 100% — it kills the whole reason for the competitor library.** The loop is **find ✅ + mine ✅ + WRITE ❌**: `genesis:paste` (the swipe-write path) **fails every time** (two "Competitor swipe" runs → `failed` / "No doc yet", **no surfaced error**), while **`genesis:brief` ("model after this: …") works.** Workaround = feed the competitor copy as a brief; the real fix = repair `genesis:paste` + surface errors. **Root cause: the swipe/remix "deconstruct" step can't find an LLM key — it reads the key from a *different location* than the main writer (a key-scoping bug); OpenRouter is configured and works everywhere else.**
...
- **🎯 Root cause (refined this session — key-scoping bug):** the swipe/remix **"deconstruct" step can't find an LLM key.** It reads the LLM key from a **different location** than the main writer — so even though an **OpenRouter key is configured and works everywhere else** (the writer runs fine), the deconstruct step looks elsewhere and fails. The error **names a specific location: dashboard Settings → Keys.**
  - **Config-vs-code test:** set an **OpenRouter or Anthropic key in dashboard Settings → Keys** (the exact location the error names), then re-run `genesis --swipe`. **Works → it's a config-location gap** (and the durable fix is to make the deconstruct step read the same key source as the writer). **Still fails with the key present → confirmed code bug for Brad.**
...
![60-competitor-swipe-failed.png](screenshots/60-competitor-swipe-failed.png)
...
- **Proposed fix:**
  1. **Fix the key-scoping bug in the swipe/remix "deconstruct" step** — make it read the LLM key from the **same source as the main writer** (where OpenRouter already works), instead of a separate location. This is the headline fix that repairs `genesis:paste`.
